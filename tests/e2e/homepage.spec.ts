...
test.describe('V12 homepage', () => {
  test('loads the complete editorial journey without a map', async ({ page }) => {
    await page.goto('./');
    ...
    await expect(page.locator('#geo')).toHaveCount(0);
  ...
  test('story reader opens and save state changes', async ({ page }) => {
    ...
    const reader = page.locator('.v12-reader');
